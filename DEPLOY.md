...
mysql:8 (内网 3306)
持久卷: data/mysql  data/qdrant_data  data/memory_db  uploads  data/letsencrypt
```
...
`glbclient.py` 与 mem0 各用一个**嵌入式 Qdrant**(`path=...` 本地文件)。嵌入式模式一个目录只能被一个进程打开,**uvicorn 必须 `--workers 1`**(Dockerfile 已固定),否则启动报 `already accessed by another instance`。
若日后要多 worker / 横向扩展,需把 Qdrant 换成独立 `qdrant/qdrant` 容器(client 改 `url=` 连法),mem0 也指向它。
...
- `docker compose ps` 三个容器应 healthy/up。
...
```
备份:定期备份 `./data/mysql`、`./data/qdrant_data`、`./data/memory_db`、`./uploads`。

---

## 验证清单

1. `docker compose ps` 三容器健康;`docker compose logs backend` 无 Qdrant `already accessed` / DB 连接错误。
