...
const cards = computed(() => [
  { title: t('nav.groups'), icon: 'ri-team-line', color: 'info', stats: c.value.groups ?? 0 },
  { title: t('nav.employees'), icon: 'ri-customer-service-2-line', color: 'success', stats: c.value.employees ?? 0 },
  { title: t('nav.activities'), icon: 'ri-flag-line', color: 'warning', stats: c.value.activities ?? 0 },
  { title: t('nav.kbs'), icon: 'ri-book-open-line', color: 'secondary', stats: c.value.kbs ?? 0 },
  { title: t('nav.materials'), icon: 'ri-image-2-line', color: 'secondary', stats: c.value.materials ?? 0 },
...
<template>
  <div>
    <h2 class="text-h5 mb-4">{{ t('dashboard.orgTitle') }}</h2>
    <VRow>
